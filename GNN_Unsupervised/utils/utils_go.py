...
def create_graph_data_other_go_features(exp, groups_id, subgroups_id, option, list_df_groups_subgroups):
    for i, group in tqdm(enumerate(groups_id)):
        list_graphs = []
        for subgroup in tqdm(subgroups_id[group]):
            df_weighted_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup), dtype={"source": "string", "target": "string"})
            G = nx.from_pandas_edgelist(df_weighted_edges, "source", "target", edge_attr="weight")
        ...
        for j, subgroup in tqdm(enumerate(subgroups_id[group])):
            ...
            for item in list(mapping.items()):
                if item[0][0] == chr(j + 65):
                    filter.append(int(item[0][1:]))
                    keys.append(item[0])
                    values.append(item[1])
            # print(filter)
            df_aux = list_df_groups_subgroups[i][j].loc[filter]
            df_aux.columns = list(range(len(df_aux.columns)))
            
            df_nodes = pd.concat([df_nodes, df_aux], ignore_index=True)
        # print(df_nodes)
        # df_nodes = pd.DataFrame(degree.items(), columns=["idx", "degree"])
        
        df_nodes.insert(0, "idx", values)
        df_nodes.insert(1, "id", keys)
        df_nodes = df_nodes.dropna(axis=1)
        # df_nodes = df_nodes.fillna(0)
        df_nodes.to_csv("{}/output/{}/preprocessing/graphs_data/nodes_data_{}_{}.csv".format(dir, exp, group, option), index=False)
...
def partial_corr(df):
    ...
    # Loop through all pairs of variables and calculate partial correlations
    for var1, var2 in combinations(columns, 2):
        covar = list(columns.difference([var1, var2]))
        # print(var1, var2, aux)
        pc = pg.partial_corr(data=df, x=var1, y=var2, covar=covar)

        list_edge.append([var1, var2, pc["r"][0]])
    df_corr = pd.DataFrame(list_edge, columns=["source", "target", "weight"])
    return df_corr

def build_graph_weight_global_partial_corr(exp, groups_id, subgroups_id, list_groups_subgroups_t, threshold=0.5):
    list_groups_subgroups_t_corr_g = []

    for i in tqdm(range(len(list_groups_subgroups_t))):
        list_aux = []
        for j in tqdm(range(len(list_groups_subgroups_t[i]))):
            df_temp = list_groups_subgroups_t[i][j]
            
            df_weighted_edges = partial_corr(df_temp)
...
def graph_partial_detail(G, id_mz=id_mz, edges=False):
    SG = G.subgraph(list(id_mz))
    ...
    print("N° nodes: ", SG.number_of_nodes())
    print("N° edges: ", SG.number_of_edges())
    plt.subplots(figsize=(5, 5))
    nx.draw_networkx(SG, pos, with_labels=True, font_color="black")
    if edges:
        edge_labels = dict([((n1, n2), round(d['weight'], 4)) for n1, n2, d in SG.edges(data=True)])
    ...
    """ df_aux = nx.to_pandas_edgelist(SG)
    print(df_aux) """
...
def std_global(dict_df_edges_filter_weight, exp, method, groups_id, option, th=0.3, plot=False, save=True):
    ...
    for group in tqdm(groups_id):
        ...
        # save
        df_edges_filter_weight.to_csv("{}/output/{}/common_edges/std_{}_{}_{}.csv".format(dir, exp, method, group, option), index=False)
...
def get_weight_global(dict_df_edges_filter, exp, groups_id, subgroups_id):
    ...
    for group in tqdm(groups_id):
        ...
        for subgroup in tqdm(subgroups_id[group]):
            df_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup),
...
def std_global_(dict_df_edges_filter_weight, exp, method, dimension, groups_id, th=0.3, plot=True, save=False):
    ...
    for group in tqdm(groups_id):
        ...
        if plot:
            ...
            t = len(df_edges_filter_weight)
            plt.title("Loss: {} of {} ({}%)".format(l, t, round(l*100/t)))
            plt.savefig("{}/output/{}/baseline/plots/common_edges_std_{}_{}_{}_{}.png".format(dir, exp, group, method, dimension, "L2"))
            # plt.show()
            plt.clf()
...
def edge_embeddings_global(exp, method, groups_id, subgroups_id, iteration):
    for group in tqdm(groups_id):
        for subgroup in tqdm(subgroups_id[group]):
            # read dataset
            df_node_embeddings = pd.read_csv("{}/output/{}/node_embeddings/node-embeddings_{}_{}_{}_{}.csv".format(dir, exp, method, group, subgroup, iteration), index_col=0)
...
def create_graph_data_(exp, groups_id, subgroups_id):
    for group in tqdm(groups_id):
        list_graphs = []
        for subgroup in tqdm(subgroups_id[group]):
            df_weighted_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup))
        ...
        common_nodes = set(list(list_graphs[0].nodes()))
        for graph in tqdm(list_graphs[1:]):
        ...
        for subgroup in tqdm(subgroups_id[group]):
            df_weighted_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup))
            G_ = nx.from_pandas_edgelist(df_weighted_edges, "source", "target", edge_attr="weight")
            G = G_.subgraph(common_nodes)
...
def create_graph_data_other_(exp, groups_id, subgroups_id, option):
    for group in tqdm(groups_id):
        list_graphs = []
        for subgroup in tqdm(subgroups_id[group]):
            df_weighted_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup))
...
def create_graph_data_go(exp, groups_id, subgroups_id, list_df_groups_subgroups):
    for group in tqdm(groups_id):
        for subgroup in tqdm(subgroups_id[group]):
            df_weighted_edges = pd.read_csv("{}/output/{}/preprocessing/edges/edges_{}_{}.csv".format(dir, exp, group, subgroup))
            G = nx.from_pandas_edgelist(df_weighted_edges, "source", "target", edge_attr="weight")
...
def build_graph_weight_global_directed(exp, list_groups_subgroups_t_corr, groups_id, subgroups_id, threshold=0.5):
    ...
    for i in tqdm(range(len(list_groups_subgroups_t_corr))):
        ...
        for j in tqdm(range(len(list_groups_subgroups_t_corr[i]))):
            ...
            df_weighted_edges = (df_temp.where(np.triu(np.ones(df_temp.shape), k=1).astype(bool)).stack())
