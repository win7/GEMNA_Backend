...
def std_global(dict_df_edges_filter_weight, exp, method, groups_id, option, th=0.3, plot=False, save=True):
    ...
    for group in tqdm(groups_id):
        ...
        # save
        df_edges_filter_weight.to_csv("output/{}/common_edges/std_{}_{}_{}.csv".format(exp, method, group, option), index=False)
...
def edge_embeddings_global(exp, method, groups_id, subgroups_id, iteration):
    for group in tqdm(groups_id):
        for subgroup in tqdm(subgroups_id[group]):
            # read dataset
            df_node_embeddings = pd.read_csv("output/{}/node_embeddings/node-embeddings_{}_{}_{}_{}.csv".format(exp, method, group, subgroup, iteration), index_col=0)
            ...
            df_edge_embeddings.to_csv("output/{}/edge_embeddings/edge-embeddings_{}_{}_{}_{}.csv".format(exp, method, group, subgroup, iteration), index=False)
...
def correlation_global(exp, groups_id, subgroups_id, list_groups_subgroups_t, method="pearson", plot=False):
    ...
    for i, list_groups in tqdm(enumerate(list_groups_subgroups_t)):
        ...
        for j, subgroup in tqdm(enumerate(list_groups)):
            ...
            matrix.to_csv("output/{}/correlations/correlations_{}_{}.csv".format(exp, groups_id[i], subgroups_id[groups_id[i]][j]), index=True)
